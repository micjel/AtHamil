...
contains
  ...
  subroutine WriteAtomicHamil(this, f)
    ...
    if(find(f,"ham.gz")) then
      call write_hamil_gzip(this, f)
      return
    end if
  ...
  subroutine write_hamil_gzip(this, f)
    ...
    do bra = 1, sps%norbs
      do ket = 1, bra
        if(sps%orb(bra)%l /= sps%orb(ket)%l) cycle
        if(sps%orb(bra)%j /= sps%orb(ket)%j) cycle
        write(buffer,"(6es20.10)") this%kinetic%m(bra,ket), this%potential%m(bra,ket), &
            & this%kinetic_p4%m(bra,ket), this%Darwin_term%m(bra,ket), &
            & this%LS_term%m(bra,ket), this%S%m(bra,ket)
        err = gzip_writeline(fp, trim(buffer), len_trim(buffer))
      end do
      ...
      do b = 1, a
        ...
        do c = 1, a
          ...
          do d = 1, c
            ...
            if( mod(oa%l+ob%l,2) /= mod(oc%l+od%l,2) ) cycle

            Jmin = max( abs(oa%j-ob%j), abs(oc%j-od%j) )/2
            Jmax = min(    (oa%j+ob%j),    (oc%j+od%j) )/2
            do J = Jmin, Jmax
              v_buffer(cnt+1) = this%ee_coulomb%GetTBME(a,b,c,d,J)
              v_buffer(cnt+2) = this%ee_darwin%GetTBME(a,b,c,d,J)
              v_buffer(cnt+3) = this%ee_spin_contact%GetTBME(a,b,c,d,J)
              v_buffer(cnt+4) = this%ee_spin_orbit%GetTBME(a,b,c,d,J)
              v_buffer(cnt+5) = this%ee_orbit_orbit%GetTBME(a,b,c,d,J)
              v_buffer(cnt+6) = this%ee_spin_dipole%GetTBME(a,b,c,d,J)
              cnt = cnt + 6
              if( cnt == 6*nbuf ) then
                do line = 1, nbuf
                  write(buffer, "(6es20.10)") v_buffer(6*(line-1)+1:6*line)
                  err = gzip_writeline(fp, trim(buffer), len_trim(buffer))
                end do
                cnt = 0
              end if
    ...
    err = gzip_close(fp)
    deallocate(v_buffer)
  end subroutine write_hamil_gzip
