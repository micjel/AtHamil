module TwoBodySpinDipole
  ...
  private :: set_ee_spin_dipole_term
  private :: ee_spin_dipole_interaction
  private :: CS_function
  private :: finalize_fnl
  private :: initialize_fnl_laguerre
  private :: set_term_integrals
  private :: get_fnl
  private :: r_power_1_2
  private :: r_power_2_1
...
contains
  ...
  subroutine set_ee_spin_dipole_laguerre( this, ms, NMesh, rmax )
    ...
    real(8) :: rmax, time
    write(*,*)
    write(*,"(a)") " Electron-electron spin-dipole: "
    ...
    time = omp_get_wtime()
    call set_ee_spin_dipole_term( this, ms )
    write(*,"(a,f12.6,a)") " Calculated ee spin-dipole: ", omp_get_wtime() - time, " sec"
    call Fintegral%finalize_fnl()
  end subroutine set_ee_spin_dipole_laguerre
  ...
  subroutine set_ee_spin_dipole_term( this, ms )
    type(TwoBodyOperator), intent(inout) :: this
    ...
    do ch = 1, ms%NChan
      ...
      do bra = 1, ms%jp(ch)%n_state
        ...
        do ket = 1, bra
          ...
          rabcd = ee_spin_dipole_interaction(oa, ob, oc, od, J)
          rbacd = ee_spin_dipole_interaction(ob, oa, oc, od, J) * (-1.d0)**((oa%j+ob%j)/2-J-1)
          rabdc = ee_spin_dipole_interaction(oa, ob, od, oc, J) * (-1.d0)**((oc%j+od%j)/2-J-1)
          rbadc = ee_spin_dipole_interaction(ob, oa, od, oc, J) * (-1.d0)**((oa%j+ob%j+oc%j+od%j)/2)
  ...
  end subroutine set_ee_spin_dipole_term

  function ee_spin_dipole_interaction(oa, ob, oc, od, J) result(r)
    use AtLibrary, only: tjs, sjs, alpha, triag
    type(EleSingleParticleOrbit), intent(in) :: oa, ob, oc, od
    integer, intent(in) :: J
    real(8) :: r
    r = ee_spin_dipole_interaction_(oa, ob, oc, od, J) + &
        & ee_spin_dipole_interaction_(ob, oa, od, oc, J)
  end function ee_spin_dipole_interaction

  function ee_spin_dipole_interaction_(oa, ob, oc, od, J) result(r)
    use AtLibrary, only: tjs, sjs, alpha, triag
    type(EleSingleParticleOrbit), intent(in) :: oa, ob, oc, od
    integer, intent(in) :: J
    real(8) :: r
    ...
    do L = Lmin, Lmax
      if( L == 0 ) cycle
      if( triag( oa%l, oc%l, L-1) ) cycle
      if( triag( ob%l, od%l, L+1) ) cycle
  ...
  subroutine initialize_fnl_laguerre(this, ms)
    ...
    call set_term_integrals(this, zeta, lmax)
  end subroutine initialize_fnl_laguerre

  subroutine set_term_integrals(this, zeta, llmax)
    ...
    do i2 = 1, NMesh
      ...
      do ket = 1, this%nidx
        ...
        do L = abs(l1-l3)-1, l1+l3+1
          if(L < 1) cycle
          if( mod(l1+l3+L,2) == 0) cycle
          if( triag(l1,l3,L-1)) cycle
          integral = 0.d0
          do i1 = 1, n_1
            r1 = r_1(i1) / zeta
    ...
    do bra = 1, this%nidx
      ...
      do ket = 1, this%nidx
        ...
        do L = lmin, lmax
          if( mod(l1+l3+L,2) == 0) cycle
          if( triag(l1,l3,L-1)) cycle
          if( mod(l2+l4+L,2) == 0) cycle
          if( triag(l2,l4,L+1)) cycle
          integral = 0.d0
          do i2 = 1, NMesh
    ...
    !$omp end parallel
    deallocate(inter)
  end subroutine set_term_integrals
